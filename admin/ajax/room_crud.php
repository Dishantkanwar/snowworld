<?php
   ...
   if(isset($_POST['add_rooms']))
   {
    ...
    $facilitie = filteration(json_decode($_POST['facilitie']));
    ...
    $q2 = "INSERT INTO `room_facilitie`(`room_id`, `facilitie_id`) VALUES (?,?)";

    if($stmt = mysqli_prepare($con,$q2)){
      foreach($facilitie as $f){
   ...
   if(isset($_POST['get_all_rooms'])){
      ...
      while($row = mysqli_fetch_assoc($res))
      {
         ...
         <td>$status</td>
         <td>
            <button type='button' onclick='edit_details($row[id])' class='btn btn-white text-white bg-black' data-bs-toggle='modal' data-bs-target='#room-edit'>
              <i class='fa-solid fa-pen-to-square'></i>&nbsp;Edit
            </button> &nbsp;&nbsp;
              <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info text-white bg-black' data-bs-toggle='modal' data-bs-target='#room-images'>
   ...
   if(isset($_POST['get_room']))
   {
     ...
     $res3 = select("SELECT * FROM `room_facilitie` WHERE `room_id`=?",[$frm_data['get_room']],'i');
     ...
     if(mysqli_num_rows($res3)>0){
      while($row = mysqli_fetch_assoc($res3)){
         array_push($facilitie,$row['facilitie_id']);
      }
     }
     $data = ["roomdata" => $roomdata, "feature"=> $feature , "facilitie"=> $facilitie];
   ...
   if(isset($_POST['edit_room']))
   {
      ...
      $del_facilitie = delete("DELETE FROM `room_facilitie` WHERE `room_id`=?",[$frm_data['room_id']],'i');

      if(!($del_feature && $del_facilitie)){
         $flag =0;
      }
   ...
   if(isset($_POST['get_room_images']))
   {
      ...
      {
         if($row['thumb']==1){
            $thumb_btn ="<i class='fa-solid fa-check bg-success text-light px-2 py-1 fs-5 rounded'></i>";
         }
         else{
           $thumb_btn ="<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary shodow-none'>
         ...
         echo<<<data
            <tr class='align-middle'>
               ...
               <td>$thumb_btn</td>
               <td>
                  <button onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-danger shodow-none'>
                  <i class='fa-solid fa-trash'></i>
                  </button>
               </td>
            </tr>
   ...
   if(isset($_POST['remove_room']))
   {
      ...
      $res4 =  delete("DELETE FROM `room_facilitie` WHERE `room_id`=?",[$frm_data['room_id']],'i');
